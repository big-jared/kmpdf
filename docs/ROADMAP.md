...
- [x] Review fixes: each line's position and width come from the text's own layout modifier (the innermost one), not the semantics node, which sits outside any padding between a semantics modifier like `testTag` and the text. The re-measure uses the width the text was drawn at, so centered and end-aligned text that doesn't wrap lines up. Leading and trailing whitespace is left out of both the text and its extent, which comes from the characters' boxes. A contract test covers all three (indented, end-aligned, centered, and padded text, each checked against its ink) and fails without each fix. Ellipsized text only includes what's visible.
...
**Verified:** JVM, iOS simulator, and Web (headless Chrome) each pass 36/36 contract tests plus every shared suite (packing 10, planning 7, margins 5, structure reader 5). JVM also passes `RasterPdfWriterTest` (12), `PdfInfoTest` (2), and `DesktopKmPdfGeneratorTest` (3). Android passed 35/35 contract tests and the 27 shared tests on an API 35 emulator before the review fixes (the Pixel 8a wasn't connected). The emulator's system server crashed under load partway through the first run, so the six contract tests that hadn't run were run again on their own, and all passed. After the review fixes, the new layout test and the five other text and reference-render tests pass on the emulator; a full re-run stopped after three tests when the emulator went offline, so the full Android suite still needs a run with the fixes. `apiCheck` and detekt pass, and the sample builds for Android, Desktop, Web, and iOS. CI (`.github/workflows/build.yml`) now runs the JVM, iOS simulator, and browser tests on macOS and the Android tests on an API 35 emulator; it hasn't run yet because nothing is pushed.
